...
analysis_methods = [
    {
        ...
        "parameters_introduction": ["最小支持度是组合至少出现的频率", "最小置信度是指Y在包含X的事务中至少出现的频率",
                                    "请注意，如果这两个值设置的过低（一般是是小于0.5），将导致算法分析异常耗时，且有可能分析失败",
                                    "请确保输入的数据至少有两个是一样的，否则一定会分析失败",
                                    "数据量可以输入较少，但是如果每行的数据量比较长（大于20个）且所有行（行数也较少的时候，如少于5行）之间极度相似，会导致内存占用过高，分析非常缓慢",
                                    ],
        "simple_description": "关联分析是一种简单、实用的分析技术，就是发现存在于大量数据集中的关联性或相关性，从而描述了一个事物中某些属性同时出现的规律和模式。",
        "detail": ["关联分析是从大量数据中发现项集之间有趣的关联和相关联系。",
    ...
    # },
    {
        ...
        "parameters_introduction": ["是否带即权重表示绘制网络图的时候是否考虑数据出现的次数", "请确保输入的数据至少有两个是一样的，否则一定会分析失败", ],
        "simple_description": "社团结构是复杂网络普遍存在的拓扑特性之一，社团发现是利用图拓扑结构中所蕴藏的信息从复杂网络中解析出其模块化的社团结构，对于研究整个网络的模块、功能及其演化，具有十分重要的意义。",
        "detail": [
            "网络是一种包含节点和边的集合，通常节点代表组成该系统的个体，边代表系统个体间的相互作用关系。相对而言，复杂网络具有节点多样性、结构复杂性、连接多样性以及网络中各种复杂性的相互影响。2002年，Girvan和 Newman等人发现复杂网络中存在一种新的结构——社团结构，其特征是同一社团内节点连接紧密，不同社团间连接稀疏。",
            "1.非重叠社团发现算法：非重叠社团发现是指识别出的社团之间互不重叠 ，每个节点有且仅属于一个社团。典型算法有模块度优化算法、谱分析法、信息论方法、标号传播方法等。基于模块度优化的社团发现算法是目前研究最多的一类算法， 其思想是将社团发现问题定义为优化问题， 然后搜索目标值最优的社团结构。2004年，Newman等人首次给出了模块度Q的定义，用以衡量社团划分结果的好坏。基于模块度优化的社团发现法通过优化目标函数Q使模块度最大化，从而得到最优的社团划分结果。",
        ...
        "parameters_introduction": ["阈值是骨干网络中将边纳入考量的最小评分值", "是否带即权重表示绘制网络图的时候是否考虑数据出现的次数",
                                    "请确保输入的数据至少有两个是一样的，否则一定会分析失败", ],
        "simple_description": "复杂网络中重要节点的影响力度量是网络信息挖掘中的关键问题，少数的初始节点就能影响绝大部分节点，通过抽取网络骨干结构能够帮助我们更好地理解网络的结构",
        "detail": [
            "现实生活中产生的数据往往呈现指数增长，海量数据的涌现使得我们必须要处理大量现实世界的复杂信息。复杂网络的节点和边的数量不断增加，这为复杂网络的研究、分析、可视化带来了不小的挑战。如何找到复杂网络中重要的、有影响力的组成部分，抽取出重要的核心骨干结构以简化网络有待解决。",
        ...
        "detail": [
            "复杂网络分析的一项重要任务就是揭示网络中的子结构,其中凝聚子群是一种典型的网络结构,完全连接的最大团结构就是紧密度最高的凝聚子群。寻找社团网络中的最大团是目前复杂网络分析的研究热点之一。通俗点讲就是在一个无向图中找出一个点数最多的完全图，即最大完全子图"],
        "description_image": "",
        "limits": {}
    }, {
        "name": "onion_layers",
        ...
        "detail": [
            "在社团内节点中，一些节点只具有该社团内节点的性质，其中一些节点在社团中发挥着重要作用。另外一些节点则由于本身度小、邻居节点度小、具有少部分其他社团性质等原因发挥的作用较小。社团中的每一个节点都有一个体现节点在社团归属程度的归属系数(belonging coefficients)。具有不同归属系数的节点处于社团的不同层次中，从而探测社团内部节点之间的层次结构关系。"],
        "description_image": "",
        "limits": {}
    }]
